...
from src.core.models import ChatMessage, PendingModification, AccumulatedParams
...
class ChatbotService:
    ...
    # -------------------------------------------------------------------------
    # Helper: format accumulated summary 
    # -------------------------------------------------------------------------
    def _format_accumulated_summary(self, itinerary_id: str, current_itinerary: Dict) -> str:
        params = self.accumulated_params.get(itinerary_id, {})  
        pass
    ...
    async def _apply_accumulated_params(
        ...
        if self.itinerary_service:
            try:
                ...
                # --- Update in‑memory context with the new itinerary ---
                ...
                response = (
                    ...
                    f"{', '.join(changes)}.\n\n"
                    f"Everything else ({', '.join}) remains unchanged."
                )
